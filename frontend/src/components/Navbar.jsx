    ...
    <nav className="sticky top-0 z-50 shadow-xl px-4 md:px-8 bg-dark1 border-b border-gray1">
      <div className="max-w-7xl mx-auto flex items-center justify-between h-16">
        ...
        <div className="hidden lg:flex items-center flex-1 max-w-md mx-8">
          <div className="relative w-full">
            <input
              ...
              className="w-full py-2 pl-10 pr-4 rounded-xl focus:outline-none transition-all border-2 border-transparent focus:border-blue1 bg-gray1 text-white1"
            />
            <svg className="absolute left-3 top-2.5 w-5 h-5 stroke-blue1" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <button className="flex items-center gap-2 p-1.5 pr-4 rounded-full bg-gray1 hover:bg-opacity-80 transition-all text-white1">
      ...
      {isOpen && (
        <div className="md:hidden pb-6 space-y-2 border-t border-gray1 animate-in fade-in slide-in-from-top-2">
          ...
          <MobileLink label="Messages" />
          <MobileLink label="Notifications" />
          <MobileLink label="My Profile" />
        </div>
      )}
    </nav>
  );
...
const MobileLink = ({ label }) => (
  <a href="#" className="block px-4 py-3 rounded-xl font-medium text-white1 hover:bg-white1 hover:bg-opacity-5">
    {label}
  </a>
);
